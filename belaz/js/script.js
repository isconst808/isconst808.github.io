...
var faqLinks = document.querySelectorAll('.faq__link'),
    faqAnswers = document.querySelectorAll('.faq-answer'),
    btnsCloseAnswer = document.querySelectorAll('.faq-answer__close'),
    classActive = 'active',
    tagBody = document.querySelector('body');
document.addEventListener('click', function (e) {
  if (!findLink(e, faqLinks)) {
    if (!findModalAnswer(e, 'faq-answer')) {
      removeClassActive(faqLinks);
      removeClassActive(faqAnswers);
    }
  }
});

for (var i = 0; i < btnsCloseAnswer.length; i++) {
  btnsCloseAnswer[i].addEventListener('click', function (e) {
    removeClassActive(faqLinks);
    removeClassActive(faqAnswers);
  });
}

function findLink(elem, arrLinks) {
  for (var index = 0; index < arrLinks.length; index++) {
    var element = arrLinks[index];

    if (arrLinks[index] == elem.target) {
      return true;
    }
  }
}

function findModalAnswer(elem, className) {
  for (var key in elem.path) {
    if (elem.path[key] != tagBody) {
      for (var index = 0; index < elem.path[key].classList.length; index++) {
        if (elem.path[key].classList[index] == className) {
          return true;
        }
      }
    } else {
...
for (var index = 0; index < faqLinks.length; index++) {
  faqLinks[index].addEventListener('click', function (e) {
    e.preventDefault();
    removeClassActive(faqLinks);
    removeClassActive(faqAnswers);
    e.target.nextElementSibling.classList.add(classActive);
...
function removeClassActive(array) {
  for (var _index = 0; _index < array.length; _index++) {
    array[_index].classList.remove(classActive);
  }
}
