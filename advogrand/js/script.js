...
var menuBurger = document.querySelector('.header-nav__burger'),
    navMenu = document.querySelector('.header-nav__body'); // Открыть меню-бургер

menuBurger.addEventListener('click', function (e) {
  menuBurger.classList.toggle(classActive);
  navMenu.classList.toggle(classActive);
  tagBody.classList.toggle(classLock);
}); // Если ширина >992px закрывать меню-бургер
...
function closeMenuBurger() {
  menuBurger.classList.remove(classActive);
  navMenu.classList.remove(classActive);
  tagBody.classList.remove(classLock);
} // Определяем высоту фиксированнной шапки
...
window.addEventListener('resize', function (e) {
  if (headerTopHeight !== headerTop.offsetHeight) {
    appPaddingTop();
  }
});

function appPaddingTop() {
  headerTopHeight = headerTop.offsetHeight;
  blockHeader.style.paddingTop = "".concat(headerTopHeight, "px");
}

appPaddingTop(); // Скролл при нажатии на пункты меню

var headerNavMenu = document.querySelectorAll('.header-nav__link');
headerNavMenu.forEach(function (element) {
  element.addEventListener('click', function (e) {
    smoothScroll(e);

    if (menuBurger.classList.contains(classActive)) {
      setTimeout(closeMenuBurger, 500);
    }
  });
});
...
function smoothScroll(event) {
  ...
  window.scrollTo({
    top: document.querySelector(targetId).getBoundingClientRect().top + document.documentElement.scrollTop - headerTopHeight,
    behavior: "smooth"
  });
}
...
;
var feedbackSlider = new Swiper('.feedback-slider__container', {
  navigation: {
...
}); // Вытащить из видео превью и поставить его вместо видео

var previewArr = document.querySelectorAll('.feedback-slide__preview');
var img = document.createElement('img');
previewArr.forEach(function (elem) {
  elem.insertAdjacentHTML('beforeend', '<img>');
  var idVideo = elem.querySelector('iframe').src.split('https://www.youtube.com/embed/')[1];
  elem.querySelector('img').src = "https://i.ytimg.com/vi/".concat(idVideo, "/sddefault.jpg");
  elem.querySelector('iframe').remove();
}); // Заменить контент модального окна в зависимости от выбранного видео

var feedbackSlideArr = document.querySelectorAll('.feedback-slide');
var modalFeedbackVideo = document.querySelector('.modal-feedback__video');
var modalFeedback = document.querySelector('.modal-feedback');
feedbackSlideArr.forEach(function (element) {
  element.addEventListener('click', function (e) {
    e.preventDefault();
    changeVideoSrc(element);
    changeFeedbackModalTitle(element);
  });
});

function changeVideoSrc(el) {
  var oldVideoLink = modalFeedbackVideo.querySelector('iframe').src;
  var oldVideoId = oldVideoLink.split('https://www.youtube.com/embed/')[1];
  var tmpVideoLink = oldVideoLink.split(oldVideoId)[0];
  var newVideoId = el.querySelector('img').src.split('https://i.ytimg.com/vi/')[1].split('/sddefault.jpg')[0];
  modalFeedbackVideo.querySelector('iframe').src = tmpVideoLink + newVideoId;
}

function changeFeedbackModalTitle(el) {
  var feedbackModalTitle = modalFeedback.querySelector('.modal-feedback__h5');
  var newFeedbackModalTitle = el.querySelector('.feedback-slide__title').innerText;
  feedbackModalTitle.innerText = newFeedbackModalTitle;
} // Фильтр категорий видео


var feedbackBtnSortArr = document.querySelectorAll('.feedback__btn-sort'),
    feedbackCatefory = document.querySelectorAll('[data-category]');
feedbackBtnSortArr.forEach(function (element) {
  element.addEventListener('click', function (e) {
    e.preventDefault();
    var datacategory = element.hash.replace(/#/, '');

    if (datacategory === 'all') {
      showAllCategory();
    } else {
      hindCategory(datacategory);
    }

    removeActiveClassFeedbackBtnCategory();
    element.classList.add(classActive);
    feedbackSlider.update();
  });
});

function hindCategory(name) {
  showAllCategory();
  feedbackCatefory.forEach(function (element) {
    if (element.dataset.category !== name) {
...
function removeActiveClassFeedbackBtnCategory() {
  feedbackBtnSortArr.forEach(function (elem) {
...
var btnTariffs = document.querySelectorAll('[data-sort]'),
    compare = 'compare',
    blockTariffs = document.querySelector('.tariffs__row');
btnTariffs.forEach(function (elem) {
  elem.addEventListener('click', function (e) {
    var dataset = e.target.dataset.sort;

    if (dataset === compare) {
      blockTariffs.classList.add(compare);
    } else {
      blockTariffs.classList.remove(compare);
    }

    removeClassCompara();
    e.target.classList.add(compare);
  });
});

function removeClassCompara() {
  btnTariffs.forEach(function (elem) {
    elem.classList.remove(compare);
  });
}
...
function showModal() {
  ...
  isModal.classList.add(classActive);
} // Определить какое модальное окно открыть
...
btnModal.forEach(function (item) {
  item.addEventListener("click", function (e) {
    ...
    showModal();
    modalContent.classList.add(classActive);
  });
